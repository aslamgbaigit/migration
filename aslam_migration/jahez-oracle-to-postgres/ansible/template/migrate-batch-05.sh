...
if [ "$1" = "" ]; then
  ...
  environment=$1
  ora2pgConfFile="conf/${environment}_no_truncate_ora2pg.conf"
fi
...
run_ora2pg() {
  ...
  ora2pg --temp_dir $oraTempDir --jobs 4 --type COPY --limit 200000 --copies 4 --allow $t --conf $ora2pgConfFile >> logs/migrate-$t.log 2>&1
  retVal=$?
  if [ $retVal -ne 0 ]; then
...
tbls="RESTAURANT_ORDER ORDER_LINE_ITEM_MIV ORDER_LINE_ITEM ORDER_LINE_ITEM_CHOICE ORDER_LINE_ITEM_OPTION JZ_ORDER_RATING THIRD_PARTY_ORDER HYPERPAY_TRANSACTIONS ACCOUNT_TRANSACTIONS ADDRESS LOST_REVENUE"
for t in $tbls; do
  run_ora2pg $t &
done
